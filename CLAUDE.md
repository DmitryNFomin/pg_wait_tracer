...
- `tests/run_all.sh`'s `KNOWN_FAILING` table (test name → tracking issue
  ...
  or investigated, not listed. A listed test still runs every time: a
  failure is counted as `known-failing`, not `failed`; a pass is loud
  ("UNEXPECTED PASS") and counts as a failure, so a fixed bug can't rot in
  the list.
